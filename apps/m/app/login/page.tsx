...
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false)
    if (res?.error) {
      setError('อีเมลหรือรหัสผ่านไม่ถูกต้อง / สิทธิ์ไม่ถึง')
      return
    }
    router.push(search.get('callbackUrl') ?? '/report/dashboard')
    router.refresh()
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <Label required htmlFor="email">อีเมลพนักงาน</Label>
        <Input
          id="email"
          type="email"
          required
      ...
      <div>
        <Label required htmlFor="password">รหัสผ่าน</Label>
        <Input
          id="password"
          type="password"
          required
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-brand-50 via-white to-brand-50/40 px-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">PMS Manager</h1>
          <p className="mt-1 text-sm text-gray-600">เข้าสู่ระบบสำหรับพนักงาน / Admin</p>
        </div>
        ...
        <p className="mt-6 text-center text-xs text-gray-500">
          Demo: <code className="rounded bg-white px-1.5 py-0.5 ring-1 ring-gray-200">admin@pms.local</code> /{' '}
          <code className="rounded bg-white px-1.5 py-0.5 ring-1 ring-gray-200">admin1234</code>
        </p>
      </div>
    </div>
  )

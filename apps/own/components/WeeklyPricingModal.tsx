    ...
    await Promise.all([
      upsert.mutateAsync({
        ...
        rows: rows.map((r, dow) => ({
          ...
          minStay: r.minStay,
          splitOpen: r.splitOpen,
          // UI no longer exposes these — mirror maxGuests into includedGuests
          // (so the base price covers all allowed guests) and drop the extra-guest fee.
      ...
      <ModalBody>
        {isPending ? (
          ...
          <div className="space-y-5">
            {/* Guest capacity — only "จำนวนท่าน" remains. "รวมในราคา" and
                "ค่าท่านเพิ่ม" were removed per design; on save we mirror
                maxGuests into includedGuests so backend math stays consistent. */}
            <div className="rounded-xl border border-gray-200 bg-white p-4">
              ...
              <div className="max-w-xs">
                <Label htmlFor="guests-count">จำนวนท่าน</Label>
                <Input
                  id="guests-count"
                  type="number"
                  min={1}

...
export function AvailabilityGrid({ hotelId }: { hotelId: string }) {
  ...
  const { data, isPending } = trpc.hotelBooking.availabilityByHotel.useQuery({ hotelId, from, to })
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button type="button" size="sm" variant="outline" onClick={() => {
            const t = new Date(); t.setUTCHours(0, 0, 0, 0); setAnchor(t)
          }}>
            วันนี้
          </Button>
          <Button type="button" size="icon" variant="outline" onClick={() => setAnchor(addDays(anchor, DAYS))}>
      ...
      {!isPending && (!data || data.length === 0) && (
        <div className="rounded-xl border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500">
          ยังไม่มีประเภทห้อง — กลับไปหน้าแก้ไขโรงแรมเพื่อเพิ่ม
        </div>
      )}
                    ...
                    {rt.days.map((day) => {
                      ...
                      return (
                        <td key={day.date} className="px-1 py-1 text-center align-middle">
                          <div className={`mx-auto flex h-9 w-9 items-center justify-center rounded-lg text-[11px] font-semibold tabular-nums ${cellClass}`}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-[10.5px] text-gray-500">
        <span className="inline-flex items-center gap-1.5"><span className="size-3 rounded bg-emerald-100 ring-1 ring-emerald-200" /> ว่าง (มากกว่า 30%)</span>
        <span className="inline-flex items-center gap-1.5"><span className="size-3 rounded bg-amber-50 ring-1 ring-amber-200" /> ใกล้เต็ม</span>
        <span className="inline-flex items-center gap-1.5"><span className="size-3 rounded bg-rose-100 ring-1 ring-rose-200" /> เต็ม</span>
      </div>
    </div>
  )
}

...
interface LineForm { roomTypeId: string; roomsReserved: number }
...
const emptyForm = (): BookingForm => ({
  ...
  guestCount: 2, checkin: tomorrow(), checkout: dayAfter(),
  lines: [{ roomTypeId: '', roomsReserved: 1 }],
  publicNote: '',
})

export function HotelBookingsManager({ params }: { params: Promise<{ id: string }> }) {
  const { id: hotelId } = use(params)
  const utils = trpc.useUtils()
  const { data: hotel } = trpc.hotel.byId.useQuery({ id: hotelId })
  const { data: roomTypes } = trpc.roomType.list.useQuery({ hotelId })

  const [tab, setTab] = useState<StatusFilter>('ALL')
  const [search, setSearch] = useState('')
  const { data: bookings, isPending } = trpc.hotelBooking.list.useQuery({
    hotelId,
  ...
  const refetch = () => {
    utils.hotelBooking.list.invalidate()
    utils.hotelBooking.availabilityByHotel.invalidate({ hotelId })
  }

  const create = trpc.hotelBooking.create.useMutation({
    onSuccess: () => { refetch(); setOpen(false); setForm(emptyForm()) },
    onError: (e) => alert(e.message),
  })
  const confirmMut = trpc.hotelBooking.confirm.useMutation({ onSuccess: refetch, onError: (e) => alert(e.message) })
  const cancelMut = trpc.hotelBooking.cancel.useMutation({ onSuccess: refetch, onError: (e) => alert(e.message) })
  ...
  const addLine = () => setForm({ ...form, lines: [...form.lines, { roomTypeId: activeRoomTypes[0]?.id ?? '', roomsReserved: 1 }] })
  const removeLine = (i: number) => form.lines.length > 1 && setForm({ ...form, lines: form.lines.filter((_, idx) => idx !== i) })
  const setLine = (i: number, patch: Partial<LineForm>) => setForm({ ...form, lines: form.lines.map((l, idx) => idx === i ? { ...l, ...patch } : l) })
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <Link href="/manage/hotels" className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href={`/manage/hotels/${hotelId}/edit`}>
            <Button variant="secondary">
              <Icon name="edit" className="size-3.5" /> แก้ไขโรงแรม / ห้อง
            </Button>
          </Link>
          <Button onClick={openCreate} disabled={activeRoomTypes.length === 0}>
      ...
      <div>
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="relative lg:max-w-xs lg:flex-1">
            ...
            <Input placeholder="ค้นหา code / ชื่อลูกค้า / เบอร์" value={search}
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {items.map((b) => {
                    ...
                    const nightsB = Math.max(1, Math.round((new Date(b.checkout).getTime() - new Date(b.checkin).getTime()) / 86_400_000))
                    return (
                      <tr key={b.id} className="hover:bg-gray-50/60">
                        ...
                        <td className="px-4 py-3 align-middle">
                          <div className="flex items-center justify-end gap-1">
                            {b.status === 'PENDING' && (
                              <button type="button" title="ยืนยัน" onClick={() => confirmMut.mutate({ id: b.id })}
                                className="flex size-8 items-center justify-center rounded-lg text-emerald-700 hover:bg-emerald-50">
                                <Icon name="check" className="size-4" />
                              </button>
                            )}
                            {(b.status === 'PENDING' || b.status === 'CONFIRMED') && (
                              <button type="button" title="ยกเลิก" onClick={() => {
                                const reason = prompt(`ยกเลิก ${b.code}? (เหตุผล — optional)`)
          ...
          <ModalBody className="space-y-5">
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input id="b-name" required value={form.customerName}
                    onChange={(e) => setForm({ ...form, customerName: e.target.value })} />
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div><Label htmlFor="b-phone">เบอร์โทร</Label>
                    <Input id="b-phone" value={form.customerPhone}
                      onChange={(e) => setForm({ ...form, customerPhone: e.target.value })} /></div>
                  <div><Label htmlFor="b-email">อีเมล</Label>
                    <Input id="b-email" type="email" value={form.customerEmail}
                      onChange={(e) => setForm({ ...form, customerEmail: e.target.value })} /></div>
                  <div><Label required htmlFor="b-guests">ผู้เข้าพัก</Label>
                    <Input id="b-guests" type="number" min={1} required value={form.guestCount}
                      onChange={(e) => setForm({ ...form, guestCount: Number(e.target.value) })} /></div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label required htmlFor="b-in">เช็คอิน</Label>
                  <Input id="b-in" type="date" required value={form.checkin}
                    onChange={(e) => setForm({ ...form, checkin: e.target.value })} /></div>
                <div><Label required htmlFor="b-out">เช็คเอาท์</Label>
                  <Input id="b-out" type="date" required value={form.checkout}
                    onChange={(e) => setForm({ ...form, checkout: e.target.value })} /></div>
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {form.lines.map((line, i) => {
                  ...
                  const sub = rt ? Number(rt.pricePerNight) * line.roomsReserved * nights : 0
                  return (
                    <div key={i} className="flex flex-wrap items-end gap-2 rounded-lg border border-gray-200 bg-gray-50/50 p-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <Select id={`l-rt-${i}`} value={line.roomTypeId}
                      ...
                      <div className="w-24">
                        ...
                        <Input id={`l-qty-${i}`} type="number" min={1} value={line.roomsReserved}
                      ...
                      <div className="w-28 text-right text-sm tabular-nums">
                        ...
                        <div className="font-semibold text-gray-900">฿{sub.toLocaleString('th-TH')}</div>
                      </div>
                      {form.lines.length > 1 && (
                        <button type="button" onClick={() => removeLine(i)} title="ลบแถว"
            ...
            <div>
              ...
              <Input id="b-note" value={form.publicNote}
            ...
            <div className="rounded-xl bg-brand-50 px-4 py-3 ring-1 ring-inset ring-brand-200">
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-medium text-brand-800">ยอดรวม (โดยประมาณ)</span>
